...
jobs = pd.read_csv('linkedin_job_posting_with_skills_ALL_merged.csv')
courses = pd.read_csv('Merged_Unique_Coursera_Courses.csv')
...
def custom_tokenizer(text):
    ...
    for skill_score in text.split(';'):
        ...
        if parts:
            ...
            tokens.extend(skill_name.split())
...
def match_jobs(user_skills, jobs_df, threshold=0.2):
...
def recommend_courses(user_skills, target_job_skills, courses_df):
    ...
    app.logger.info(missing_skills)
    ...
    for skill, score in missing_skills.items():
        ...
        app.logger.info(first_relevant_course.head())
...
def get_job_details():
    ...
    results = []
    app.logger.info(user_skills)
    app.logger.info(job_ids)
    app.logger.info(jobs_df.head())
    for jid in job_ids:
